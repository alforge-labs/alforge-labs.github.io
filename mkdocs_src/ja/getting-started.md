...
| `forge.yaml が見つかりません` / `Config file not found` | カレントディレクトリに `forge.yaml` がありません。プロジェクト作業ディレクトリで `alpha-forge system init` を実行するか、`FORGE_CONFIG=/path/to/forge.yaml alpha-forge ...` の形で環境変数を指定してください。 |
