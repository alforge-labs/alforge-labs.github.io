...
!!! tip "表示言語は環境のロケールから自動判定されます"

    CLI の表示言語（EULA 同意プロンプトを含む）は `LC_ALL` / `LC_MESSAGES` / `LANG` を見て決まります。日本語ロケールなら日本語、それ以外の言語のロケールなら英語です。ロケールが未設定、または `C` / `POSIX` の場合は日本語になります。

    明示的に切り替えたい場合は `FORGE_LANG` を設定してください（ロケールより優先されます）。

    ```bash
    export FORGE_LANG=ja   # 日本語に固定
    export FORGE_LANG=en   # 英語に固定
    ```

    **なお EULA の本文（全文）は英語版のみです。** 同意プロンプトは要約と全文ファイルの場所を案内します。

    ロケールを `en_US.UTF-8` にしている日本語話者は表示が英語になります。日本語に戻すには `FORGE_LANG=ja` を設定してください。
...
| エラーメッセージ / 症状 | 原因と対処 |
|------------------------|-----------|
| 環境がうまく動かない（PATH / データ / 設定） | まず `alpha-forge system doctor` を実行してください。PATH・データディレクトリ・設定ファイルの状態を診断し、未初期化なら `alpha-forge system init`、同梱データで試すなら `alpha-forge demo` といった**次に打つコマンド**を併記します。 |
| `command not found: forge` / `command not found: alpha-forge` | 新しいターミナルを開く代わりに、**現在のシェルで今すぐ使う**なら `export PATH="$HOME/.local/bin:$PATH"`（Windows PowerShell は `$env:PATH = "$env:PATH;$env:LOCALAPPDATA\Programs\alpha-forge"`）を実行します。恒久設定はインストーラが `~/.bashrc` / `~/.zshrc` に追記済みなので `source ~/.bashrc` / `source ~/.zshrc` や新しいターミナルでも有効です。それでも出る場合は `ls ~/.local/bin/alpha-forge` でバイナリの存在を確認し、`echo $PATH` に `~/.local/bin` が含まれているか確認してください。 |
...
新しいバージョンがあるとき、コマンドの実行後に 1 行の案内が表示されます。

```text
ℹ 新しいバージョンがあります: 1.2.0 → 1.3.0
  `alpha-forge self update` で更新できます（FORGE_NO_UPDATE_CHECK=1 でこの案内を無効化）。
```

- 最新バージョンの確認は **24 時間に 1 回**だけ行い、結果をキャッシュします。オフラインでも長くは待たされません（接続 1.5 秒・読み取り 2.0 秒でタイムアウトし、確認に失敗しても黙って次回に回します）
- 同じバージョンについて案内が出るのは **24 時間に 1 回**までです。新しいバージョンが出たときはその場で案内します
- 以下の場合は確認も表示も行いません
    - `--json` を付けた実行（機械が読む出力を汚さないため）
    - 非対話実行（`FORGE_NONINTERACTIVE=1` / `CI=1` / 標準入力が TTY でない場合）
    - 開発モード（ソースからの実行）— `alpha-forge self update` が使えないため
- プレリリース（beta 等）は案内しません。プレリリースを使う場合は `alpha-forge self update --version <タグ>` でバージョンを明示してください

案内を無効化するには環境変数を設定してください。

```bash
export FORGE_NO_UPDATE_CHECK=1
```

確認状況は `$XDG_CONFIG_HOME/forge/update_check.json`（未設定時 `~/.config/forge/update_check.json`）に記録されます。

!!! info "送信先について"

    更新確認の通信先は **GitHub（配布リポジトリの Releases API）** です。AlphaForge の開発者側には何も送信されません。バージョン番号・利用状況を含むテレメトリは一切ありません。
...
うまくいかなかったこと・欲しい機能・使ってみた感想は、次のコマンドで送れます。

```bash
alpha-forge feedback
```

ブラウザで [alforge-labs/feedback](https://github.com/alforge-labs/feedback) の issue フォームが開きます。直近 24 時間以内にエラーが出ていた場合は、バグ報告テンプレートが選ばれ、次の項目が自動で入ります。

| 自動で入るもの | 決して入らないもの |
|---|---|
| AlphaForge のバージョン | **コマンドの引数**（銘柄・戦略 ID） |
| OS / アーキテクチャ | **診断ログの中身** |
| エラーの種類（例外クラス名） | |
| サブコマンド名（引数を除く） | |
| 診断ログの**パス** | |

!!! info "送信ボタンを押すまで何も送信されません"

    自動で入るのは上表の左側だけです。**銘柄や戦略 ID を含むコマンド引数と、診断ログの中身は含まれません。** 内容は送信前に自由に編集・削除できます。診断ログを添付するかどうかは、中身を確認したうえでご自身で判断してください。

ブラウザを開かず URL だけ確認したい場合は `alpha-forge feedback --print-url` を使ってください。非対話実行（`FORGE_NONINTERACTIVE=1` / CI / 標準入力が TTY でない場合）でも同じ挙動になります。
