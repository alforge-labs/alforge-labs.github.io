...
v0.9.0 から、alpha-visualizer は「見るだけ」のビューアではなく、**バックテスト・最適化・Walk-Forward Test をブラウザから実行できる GUI** になりました。サーバーと同じマシンに AlphaForge CLI がインストールされていることが前提です（CLI が無い環境では従来どおり閲覧専用として動作します）。
