...
- **レンジ端の選び方に注意**: `spread_pct_range` には**実在する業者の水準**を入れてください（理論上の下限や安全マージンではなく）。業者は均等に散らばるとは限らず、実測では掲示の大半が狭い帯に集中し、離れた1件が「数倍の開き」を作るケースがあります。**レンジの端が1社だけで決まっていないかは、最も低い（高い）1件を除いたときに幅がどれだけ縮むかで判別できます**（実測例では、あるペアの幅が 9.50円 → 1.50円 と 84% 縮んだのに対し、別のペアは下端に3社が並ぶため 2.00円 のまま変わりませんでした）。1件外しただけで大きく縮む端は、その1社に張り付いています。ただしその1社が長期にわたって同じ水準に居続けるかどうかは別問題で、数週間の観測から10年を仮定することはできません。low を「あり得る最悪ケース」と読むか「その業者を選んだ場合の答え」と読むかは、この点を踏まえて判断してください。どちらを取ったかは出力の `carry_spread_scenarios`（使用 spread 値）で確認できます
- **レンジの定数化も近似です**: 業者間レンジ自体が時間とともに変動します（スワップの改定は頻繁で、業者間の幅が数週間で大きく縮むこともあります）。長期バックテストに spread の時系列を持ち込むのは現実的でないため、本機能は「一定レンジの上下で回して結論が頑健かを見る」ための感度分析として使ってください（特定時点の実測レンジを固定した近似です）
