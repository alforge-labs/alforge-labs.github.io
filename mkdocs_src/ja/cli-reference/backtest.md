...
- **レンジ端の選び方に注意**: `spread_pct_range` には**実在する業者の水準**を入れてください（理論上の下限や安全マージンではなく）。業者は均等に散らばるとは限らず、実測では掲示の大半が狭い帯に集中し、離れた1件が「数倍の開き」を作るケースがあります。この離れた1件は一時的な外れ値とは限らず、複数週にわたって同じ水準に居続ける例が観測されています（上端・下端とも同じ業者のまま開きが変わらない）。したがって low は「捨ててよい極端値」ではなく「**その業者を選んで持ち続けた場合の答え**」です。多数派の帯の端を取るか、離れた業者の水準まで含めるかで low の意味が変わるため、どちらを取ったかは出力の `carry_spread_scenarios`（使用 spread 値）で必ず確認してください
