...
| 名前 | 種別 | デフォルト | 説明 |
|------|------|----------|------|
| `--symbol` / `-s` | 文字列（複数可） | なし | 対象シンボル。複数指定・カンマ区切りの両方に対応（issue #1392） |
| `--interval` | 文字列 | なし（`--common-range` 時は `1d`） | 時間足で絞り込む（issue #1392） |
| `--common-range` | フラグ | false | 指定銘柄の共通期間と、期間を制約している銘柄を表示する（issue #1392） |
...
### 複数銘柄の共通期間（`--common-range`）

複数銘柄を横断する検証で窓を決めるとき、対象の一部だけ価格データが短いと「短い窓を長い窓のつもりで比べる」誤りが起きます。`--common-range` は共通期間に加えて **どの銘柄がその期間を制約しているか** を出します。
...
```text
共通期間: 2021-03-23 〜 2026-08-24（1410 rows・1d）
  開始を制約: AUDJPY=X（2021-03-23 開始・最も早い銘柄より 8910 日遅い）
  開始を制約: CADJPY=X（2021-03-23 開始・最も早い銘柄より 8910 日遅い）
  全期間が使える銘柄: USDJPY=X（1996-10-30〜2026-08-24） / EURJPY=X（2003-01-23〜2026-08-24）
```

制約している銘柄が分かれば、「その銘柄を落として長い窓を取る」か「短い窓で全銘柄を使う」かを選べます。`rows` は暦日数ではなく**全銘柄が値を持つ日付の数**です。未取得の銘柄があると黙って除外せず exit code 2 で停止します（欠けたまま窓を決めると判断を誤るため）。データの自動補完・取得や、バックテスト側での自動除外は行いません。
...
- **1 日あたりへの正規化は必須の規約**: 業者の掲示値には付与日数がまとめて乗っています（水曜に 3 日分など）。掲示値をそのまま入れると計上時のカレンダー日数換算と重なって**二重計上**になります。必ず付与日数で割ってから取り込んでください（取込時に同一列内で中央値の 2.5 倍を超える飛び値を検出すると「付与日数込みの可能性」を警告します）
- 日付は昇順・重複なし・将来日なし。空セル（欠損値）不可
- ロング/ショートは別列で、どちらも**マージン込みネット値**（`spread_pct` 非適用）
- 計上はカレンダー日ベースの近似のため、**週の途中で対象期間を切ると実際の付与累積と数日ぶんずれます**（短い窓ほど相対的に影響大。週の境界で切るのが安全）
