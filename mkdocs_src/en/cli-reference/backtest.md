...
- **Choose the range endpoints deliberately**: put **real broker levels** into `spread_pct_range` (not theoretical floors or safety margins). Brokers do not spread out evenly: in measured data, most quotes clustered in a narrow band while one distant quote created a "several-fold spread". That distant quote is not necessarily a transient outlier — it has been observed staying at the same level for weeks (the same brokers holding both endpoints with an unchanged gap). The low scenario is therefore not an "extreme value to discard" but "**the answer you get if you pick that broker and hold**". Whether low means the edge of the majority band or the level of a persistently distant broker changes how to read the results, so always check the actual spread values used via `carry_spread_scenarios` in the output
