...
| `--dividend-reinvest` | flag | false | Include dividend-reinvest metrics in the result (#958); requires saved dividends data (`data fetch --with-dividends`). When set, `--json` includes `dividend_adjusted_metrics` (dividend-reinvested total_return_pct / max_drawdown_pct / cagr_pct) and `dividend_adjusted_note`, and the text output shows the same block. Note: closes stored by `data fetch` for ETFs and stocks are dividend-adjusted, so applying `--dividend-reinvest` to stored data double-counts dividends. |
...
- **Choose the range endpoints deliberately**: put **real broker levels** into `spread_pct_range` (not theoretical floors or safety margins). Brokers do not spread out evenly: in measured data, most quotes clustered in a narrow band while one distant quote created a "several-fold spread". **You can test whether an endpoint is pinned to a single broker by checking how much the width shrinks when you drop the single lowest (or highest) quote** (in one measured example a pair's width shrank from 9.50 yen to 1.50 yen — an 84% collapse — while another pair stayed at 2.00 yen because three brokers shared the bottom quote). An endpoint that collapses after dropping one quote is pinned to that broker. Whether that broker stays at the same level over the long run is a separate question — a few weeks of observation cannot justify a 10-year assumption. Decide with this caveat in mind whether to read the low scenario as a "plausible worst case" or as "the answer you get if you pick that broker". Which one you chose remains traceable via `carry_spread_scenarios` (the spread values used) in the output
- **The constant range is itself an approximation**: the broker range moves over time (swap points are revised frequently, and the width between brokers can shrink substantially within weeks). Carrying a spread time series through a long backtest is impractical, so use this feature as a sensitivity analysis — checking whether the conclusion is robust across a fixed range — rather than a model of actual spreads (it freezes a range measured at one point in time)
...
| `--end` | option | - | End date `YYYY-MM-DD` |
| `--rebalance` | choice | `none` | How often to reset weights to target (`none` / `monthly` / `quarterly` / `annual`). `none` sums the per-symbol curves as usual. Others rebalance to target allocation at the close of the last business day of each period. **No transaction cost is applied**. Each asset's daily returns come from its stand-alone backtest and the same return series is reused after every reset (an approximation that assumes per-asset performance does not depend on the capital allocated). |
| `--json` | flag | false | Output as JSON |
