...
| `command not found: forge` / `command not found: alpha-forge` | To **use it right away in the current shell** (instead of opening a new terminal), run `export PATH="$HOME/.local/bin:$PATH"` (Windows PowerShell: `$env:PATH = "$env:PATH;$env:LOCALAPPDATA\Programs\alpha-forge"`). The installer already appended the permanent entry to `~/.bashrc` / `~/.zshrc`, so `source ~/.bashrc` / `source ~/.zshrc` or a new terminal also work. If it still fails, confirm the binary exists with `ls ~/.local/bin/alpha-forge` and that `echo $PATH` includes `~/.local/bin`. |
